...
class SolendClaim {
    ...
    getExerciseIxs(amount) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const setupIxs = [];
            const exerciseIxs = [];
            const claimantTokenAccountAddress = yield (0, spl_token_1.getAssociatedTokenAddress)(optionMarket.underlyingAssetMint, this.provider.wallet.publicKey, true);
            ...
            if (!claimantTokenAccountInfo) {
                const createUserTokenAccountIx = (0, spl_token_1.createAssociatedTokenAccountInstruction)(this.provider.wallet.publicKey, claimantTokenAccountAddress, this.provider.wallet.publicKey, optionMarket.underlyingAssetMint);
                setupIxs.push(createUserTokenAccountIx);
            }
            const exerciserOptionTokenSrc = yield (0, spl_token_1.getAssociatedTokenAddress)(optionMarket.optionMint, this.provider.wallet.publicKey);
            const underlyingAssetDest = yield (0, spl_token_1.getAssociatedTokenAddress)(optionMarket.underlyingAssetMint, this.provider.wallet.publicKey);
            const quoteAssetSrc = yield (0, spl_token_1.getAssociatedTokenAddress)(optionMarket.quoteAssetMint, this.provider.wallet.publicKey);
            if (!optionMarket) {
                throw new Error("Option market with that key is not found.");
            }
    ...
    getClaimIxs() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const setupIxs = [];
            const claimIxs = [];
            const claimantTokenAccountAddress = yield (0, spl_token_1.getAssociatedTokenAddress)(this.metadata.distributor.mint, this.provider.wallet.publicKey, true);
            const claimantTokenAccountInfo = yield this.provider.connection.getAccountInfo(claimantTokenAccountAddress);
            if (!claimantTokenAccountInfo) {
                const createUserTokenAccountIx = (0, spl_token_1.createAssociatedTokenAccountInstruction)(this.provider.wallet.publicKey, this.metadata.distributor.mint, this.provider.wallet.publicKey, claimantTokenAccountAddress);
                setupIxs.push(createUserTokenAccountIx);
            }
